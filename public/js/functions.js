...
export function getAccount (elementID, alert)  {
    ...
    updateID.forEach((updateOne) => {
        updateOne.addEventListener('click', async () => {
            ...
            const response = await fetch(`http://localhost:4000/api/accounts-register/${id}`)
            const json = await response.json()   
...
export function updateAccount(idForm, formInput, alert) {
    idForm.addEventListener('submit', async (e) => {
        ...
        const response = await fetch(`http://localhost:4000/api/accounts-register/${formInput[4].value}`, {
            method: 'PATCH', 
            headers: {
                'Content-Type': 'application/json',
            },
